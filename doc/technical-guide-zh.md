...
- [第 3 章：数据流分析](#第-3-章数据流分析)
- [第 4 章：关键技术决策](#第-4-章关键技术决策)
- [第 5 章：模块设计详解](#第-5-章模块设计详解)
...
**路径 A**（上方）解决"AI 后端"问题：消息通道收到用户请求后，Gateway 以 OpenAI 格式 POST 到 Streaming Proxy。Proxy 自动从消息元数据推导 session key（meta → key → `--resume`），复用 cursor-agent 会话，实时流式返回 AI 响应。

**路径 B**（下方）解决"工具调用"问题：cursor-agent 推理过程中需要外部工具时，通过 MCP 协议调用 MCP Server，后者将请求转发到 Gateway REST API。MCP Server 指令中嵌入了丰富的工具描述（`extractSkillBrief`），使 LLM 可以直接调用而非先查文档。
...
```mermaid
flowchart TB
    subgraph orchestration ["编排层 (Gateway 启动时运行)"]
    ...
    subgraph runtime ["运行时 (常驻进程)"]
        Proxy["streaming-proxy.mjs<br/>HTTP :18790 / Session 管理<br/>SSE 流 / 工具调用日志"]
        MCPServer["server.mjs<br/>工具发现 / 富指令<br/>超时重试 / 缓存"]
    end
...
<img src="technical-guide-zh-mermaid/Untitled-3.png" alt="完整请求生命周期" width="2293" />
...
<img src="technical-guide-zh-mermaid/Untitled-4.png" alt="工具调用链路" width="1396" />
...
<img src="technical-guide-zh-mermaid/Untitled-5.png" alt="Session 复用流程" width="506" />
...
flowchart TD
    ...
    HasCursor -->|"有（历史会话）"| Resume["spawn --resume sessionId<br/>加载 store.db 历史"]
    ...
    AgentRun --> OutputExit["返回响应<br/>子进程退出"]
```
...
<img src="technical-guide-zh-mermaid/Untitled-6.png" alt="完整存储时序" width="1531" />
...
sequenceDiagram
    ...
    participant AgentProc as cursor-agent (子进程)
    ...
    AgentProc->>StoreDB: 读取 blobs 表<br/>加载对话历史
    StoreDB-->>AgentProc: 历史消息 + 工具调用记录

    AgentProc->>AgentProc: LLM 推理（含历史上下文）

    AgentProc-->>Proxy: stdout: session_id 事件
    Proxy->>SessionFile: 更新 "conv-123" → 新 session_id

    AgentProc-->>Proxy: stdout: text/result 事件
    AgentProc->>StoreDB: 写入新的对话记录
    Note over AgentProc: 子进程退出
...
<img src="technical-guide-zh-mermaid/Untitled-7.png" alt="Proxy 启动/重启流程" width="489" />
...
flowchart TD
    ...
    ExitEvent -->|"code 非 0"| CheckUptime{"运行时长 > 5min?"}
    ...
    CheckCount -->|"否"| AutoRestart["指数退避重启<br/>2s / 10s / 60s"]
    AutoRestart --> Kill
```
...
### 4.3 富工具描述 vs openclaw_skill 调用
...
<img src="technical-guide-zh-mermaid/Untitled-8.png" alt="register 生命周期" width="580" />
...
带重试的模型发现机制：执行 `cursor-agent --list-models`，解析输出 `"model-id  -  Model Name (annotation)"` 格式，支持最多 2 次重试。每次重试间使用 `Atomics.wait` 阻塞等待 2 秒，零 CPU 开销、无子进程、跨平台兼容。
...
<img src="technical-guide-zh-mermaid/Untitled-9.png" alt="工具发现与注册" width="574" />
...
```mermaid
flowchart TD
    subgraph p1 ["阶段 1: 候选工具发现 (磁盘 IO, 60s 缓存)"]
        ...
        ScanSkills --> Merge["合并去重"]
        ScanSource --> Merge
    end

    subgraph p2 ["阶段 2: 即时 MCP 注册 (不依赖 Gateway)"]
        direction LR
        DynTools["动态工具 (per-tool)<br/>feishu_doc / feishu_wiki / ...<br/>schema: action + args_json"]
...
所有候选工具立即通过 `server.tool(name, description, schema, handler)` 注册到 MCP Server，无需等待 Gateway 探活。这避免了 MCP Server 先于 Gateway 启动时的竞争条件。每个工具的 schema 统一为 `{ action?: string, args_json?: string }`。如果工具被调用时 Gateway 尚未就绪，`invokeGatewayTool` 内置的重试逻辑（2 次重试 + 退避）会自动处理。
...
<img src="technical-guide-zh-mermaid/Untitled-10.png" alt="事件流处理" width="1321" />
...
flowchart TD
    ...
    Parse["JSON.parse 逐行解析"]
    Switch{"event.type?"}

    ToolBranch{"subtype?"}
    ToolStart["tool:start 日志<br/>记录名称 / 参数 / call_id<br/>开始计时"]
    ToolDone["tool:done 日志<br/>耗时 / 成功 / 失败"]

    ThinkBranch{"FORWARD_THINKING?"}
    ForwardThink["SSE: reasoning_content"]
    DropThink["丢弃"]

    TextDelta["SSE: content delta<br/>(实时推送)"]

    ResultBranch{"INSTANT_RESULT?"}
    InstantSend["一次性发送<br/>零延迟"]
    ChunkedSend["分块流式<br/>~200 chars/s"]

    SaveSession["setSession<br/>持久化到磁盘"]
    ...
    Switch -->|"含 session_id"| SaveSession
```
...
### 5.5 健康检查 (src/doctor.ts)

`runDoctorChecks()` 实现 11 项检查：
