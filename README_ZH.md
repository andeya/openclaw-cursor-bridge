...
npm pack
# 生成 openclaw-cursor-bridge-<version>.tgz（约 12KB）
```
...
# 查看当前配置状态
openclaw cursor-bridge status
```

## 配置选项

在 `openclaw.json` 中 `plugins.entries.cursor-bridge.config` 下可配置：
...
| 命令                                      | 说明                                        |
| ----------------------------------------- | ------------------------------------------- |
| `openclaw cursor-bridge setup`            | 手动重新执行配置（写入 mcp.json、CLI 后端） |
| `openclaw cursor-bridge doctor`           | 检查所有组件健康状态                        |
| `openclaw cursor-bridge status`           | 显示当前配置详情                            |
| `openclaw cursor-bridge upgrade <source>` | 一键升级（清理 → 卸载 → 安装新版本）        |
| `openclaw cursor-bridge uninstall`        | 一键完整卸载（清理配置 + 删除插件文件）     |
...
2. 删除插件目录 `~/.openclaw/extensions/cursor-bridge`
3. 清理自定义配置（见下表）
4. 提示重启 gateway
...
| `openclaw.json` `plugins.entries`             | 移除 `cursor-bridge` 注册               |

> **注意**：不要直接执行 `openclaw plugins uninstall cursor-bridge`，它只移除配置条目，不会清理上表中的自定义配置。
