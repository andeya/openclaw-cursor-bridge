...
| **plugins.allow / plugins.entries: plugin not found**        | Stale config references the uninstalled plugin; `openclaw doctor --fix` may not remove it. From the project dir run **`npm run clean-config`** (or `node scripts/clean-openclaw-config.mjs`), then `openclaw plugins install ./`. Or edit `~/.openclaw/openclaw.json`: remove `openclaw-cursor-brain` from `plugins.allow` and delete `plugins.entries.openclaw-cursor-brain`.                                                                                                                            |
...
| **Cursor rate limit / 504 timeout / proxy keeps exiting**    | When Cursor is rate-limited, requests slow down and can trigger proxy "exit after consecutive timeouts". Defaults are now relaxed: 3 min degraded timeout, exit only after 5 consecutive timeouts or 8 failures. If you still get 504, set env: `CURSOR_PROXY_DEGRADED_TIMEOUT=300000`, `CURSOR_PROXY_MAX_CONSECUTIVE_TIMEOUTS=10`, then `openclaw gateway restart`. Channels like Feishu often have ~50–60s reply timeout; if Cursor is often slower, reduce concurrency or wait for rate limit to ease. |
